...
\usepackage{xcolor}         % Should load before tcolorbox                  
...
%% * When using minted in document class standalone, we do not see the line numbers which we usually want
%%   since they lie outside of the area displayed by standalone (we see this if we increase the standalone margin
%%   parameter but then the remaining part of the document is too large
%% * Therefore we need a small patch.
%% * We want to make this patch available as part of an editable ParsifalTemplate and not just hardcode it
%%   into php code.
%% * However we cannot execute this in situations where we do not load the minted package
%% * Therefore we define a (delayed executing) macro \initializeMinted here and - in case the user sets the minted
...
%%
\def\initializeMinted{
  \tcbuselibrary{breakable}
  \tcbuselibrary{minted}
  \renewcommand\theFancyVerbLine{\footnotesize\arabic{FancyVerbLine}}
  \BeforeBeginEnvironment{minted}{\begin{tcolorbox}[boxsep=10pt,boxrule=0pt]}
...
\NewEnviron{OPT}[1]{%    capitalized: initially visible
  ...
  \BODY%
  \end{ocg}%
}\begin{document}\begin{minipage}{15cm}\myInitialize\relax \typeout{START-MARKER-TYPED-OUT-FOR-ERROR-PARSER}
  
\begin{opt}{namex}initially invisible\end{opt}
  
\begin{OPT}{namex}initially visible\end{OPT}

\typeout{END-MARKER-TYPED-OUT-FOR-ERROR-PARSER}\end{minipage}\newwrite\yposoutputfile\openout\yposoutputfile=\jobname.ypos\pdfsavepos\write\yposoutputfile{\the\pagetotal}\end{document}