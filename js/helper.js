...
// TODO: we are in the process of removing helper.js from PARSIFAL and shifting this to runtime.js
...
const PARSIFAL = window.PARSIFAL = (() => {

// return true if the page contains content with a tag requiring Parsifal tex activity
// TODO: do we really want to detect this client side or server side ???
const pageContainsTex = () => { 
  const EDITBOX = document.getElementById ("wpTextbox1"); 
  if (EDITBOX) {  if ( ! REG_ANY.test (EDITBOX.value) ) { console.log ("Parsifal: Found nothing to tex in page ", REG_ANY); return false;} else {return true;}}
  else         { console.warn ("Parsifal:helper.js: found no editbox in pageContainsTex - should not happen at this stage"); return false;} 
};
...
const initializeTextarea = () => { 
  ...
  console.info ("------------------ initializeTextarea");
  ...
  const wasResized = () => {
    ...
    const VERBOSE = true;
    if (VERBOSE) {console.log ("received a resize event");}
  ...
  Object.assign (textarea.style, { minWidth: "150px", resize: "both", display:"inline-block" } );
  ...
  Object.assign (newEditContainer.style, {display: "flex"} );   
  ...
  Object.assign (previewContainer.style, {minWidth: "18px", border: "1px solid red", display: "inline-block", flex:"1 1 auto", "box-sizing": "border-box", position: "relative"} ); 
  ...
  window.setTimeout ( () => {processEdit ( "INIT" )} , 0);                                                 // kick off display of first preview
}


// code already migrated to DantePresentations DONE
...
return {pageContainsTex, editPreviewPatch};

})();



/*** COLLAPSIBLES of PARSIFAL ***/   /// THIS is ALREADY MIGRATED TO runtime.js
// #region
  ...
  showLogLinks (hash);                                                                    // show links to log file of the Tex run   
   ...
   if (errorStatus == "Hard") {     console.error ("receivedEndpointResponse: received X-Parsifal-Error = Hard. NO result to display. Will display error message instead");
     ...
     return;
      ...
      ERROR.showError ( errorDetails, "Soft");   // WE MIGHT request some error information 
    }
...
function processEdit ( tag ) {
  console.info ("***-----------------------** processEdit was invoked, tag = ", tag);
...
function processEditDoNG (resolve, reject) {
  const VERBOSE = true;
  const DETAILS = false;
  const EDITBOX = document.getElementById ("wpTextbox1");  // pick up edit box locally and right now (we had a race condition somewhere)

  if (!EDITBOX)                     { if (VERBOSE) {console.log ("Parsifal: processEditDoNG returning, no editBox");}                return;}    // there is no editbox, return  
  if (!PARSIFAL.pageContainsTex())  { if (VERBOSE) {console.log ("Parsifal: processEditDoNG returning, contains no relevant tag");}  return;}    // there is no tex-tag on this page
